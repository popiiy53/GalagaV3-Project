                                                   ...
                                                   availableGuardEnemiesIds_ { 0 }
{
    ...
    isNumberOfHitsLabelVisible_ = false;
    isNumberOfHitsNumberLabelVisible_ = false;
    isBonusLabelVisible_ = false;
    isPerfectLabelVisible_ = false;
...
void ol::GameController::draw(Renderer& renderer)
{
    ...
    else if (isCurrentLevelLabelVisible_ && hudInfo.isChallengingStage())
    {
        renderer.drawText1("CHALLENGING STAGE",
                           Vec2(44.0f, 128.0f), Color::CYAN);
    }
    else if (isCurrentLevelLabelVisible_ && !hudInfo.isChallengingStage())
    {
    ...
    {
        if (isNumberOfHitsLabelVisible_)
        {
            std::string numberOfHitsNumberStr = isNumberOfHitsNumberLabelVisible_ ?
                                                std::to_string(challengingStageTotalEnemiesHit_) : "";

            std::string numberOfHitsStr = "   " + numberOfHitsNumberStr;
            numberOfHitsStr = numberOfHitsStr.substr(numberOfHitsStr.length() - 3, 3);
            renderer.drawText1("NUMBER OF HITS " + numberOfHitsStr, Vec2(40.0f, 128.0f), Color::CYAN);
        }
        if (isBonusLabelVisible_)
        {
            if (isPerfectLabelVisible_)
            {
                renderer.drawText1("SPECIAL BONUS 10000 PTS", Vec2(20.0f, 148.0f), Color::YELLOW);
            }
            else
            {
                std::string bonusStr = "     " + std::to_string(100 * challengingStageTotalEnemiesHit_);
                bonusStr = bonusStr.substr(bonusStr.length() - 5, 5);
                renderer.drawText1("BONUS " + bonusStr, Vec2(68.0f, 148.0f), Color::CYAN);
            }
        }
        if (isPerfectLabelVisible_ && blink())
        {
            renderer.drawText1("PERFECT !", Vec2(76.0f, 108.0f), Color::RED);
        }
    }
}
...
void ol::GameController::onFighterRescued()
{
    ...
    int state = 0;
    // bool nextLife = false;   // comment out by tchung
    setUpdateAction([=] () mutable
    {
        switch (state)
        {
...
void ol::GameController::onEnemyHit(Enemy* enemy)
{
    ...
    // in challenging stage, was all enemies team destroyed ?
    if (hudInfo.isChallengingStage())
    {
        challengingStageTotalEnemiesHit_++;
        if (--challengingStageEnemiesTeamCount_[enemy->getTeamId()] == 0)
        {
            scene_.spawn<ScorePoint>()->show(3000, enemy->getPosition());
            hudInfo.addScore(3000);
        }
    }
...
void ol::GameController::spawnNextEnemies()
{
    if (!hudInfo.isChallengingStage())
    {
        ...
        for (auto enemyInfo : enemiesGroup[currentEnemiesGroup_])
        {
            ...
            float divingProbability = hudInfo.stage == 1 ? 0.0 : 0.15f;
    ...
    // challenging stage
    {
        //log("creating enemy challenging stage begin");
        std::vector<Vec2> randomPath = createRandomChallengingStagePoints();
        int enemyType = ol::random(2, 8);
        for (int i = 0; i < 8; i++)
        {
            Enemy* enemy = scene_.spawn<Enemy>();
            enemy->setFormationPosition(1, enemyType);
            enemy->move(randomPath, i * 16.0f);
            enemy->setTeamId(currentEnemiesGroup_);
            challengingStageEnemiesTeamCount_[currentEnemiesGroup_]++;
            challengingStageTotalEnemiesCount_++;
        }
    }
...
void ol::GameController::showCurrentLevel()
{
    ...
    setUpdateAction([=] () mutable {
        switch (state)
        {
            ...
            case 1:
                if (getGameElapsedTime() - waitTime < 1.0f)
                {
                    return;
                }
                ...
                if (hudInfo.isChallengingStage())
                {
                    challengingStageEnemiesTeamCount_.clear();
                    challengingStageTotalEnemiesCount_ = 0;
                    challengingStageTotalEnemiesHit_ = 0;
                    playSound(Sound::CHALLENGING_STAGE_SOUND);
                }
...
void ol::GameController::spawnChallengingStageEnemies()
{
    ...
    setUpdateAction([=] () mutable {
        ...
        switch (state)
        {
            case 0:
                ...
                else if (isAllEnemiesDestroyed())
                {
                    evaluateChallengingStage();
                    return;
                }
                waitTime = getGameElapsedTime();
                state = 1;
            ...
            case 1:
                if (getGameElapsedTime() - waitTime < 3.0f || !isAllEnemiesDestroyed())
                {
                    return;
                }
                state = 0;
        ...
        switch (state)
        {
            case 0:
                if (enemiesInFormationIds_.size() > 0)
                {
                    ...
                    {
                        if (!enemy->isMorphing() && ol::random() < 0.05f)
                        {
                            enemy->startMorphing();
                            playSound(Sound::ENEMY_METAMORPHOSE);
                        }
                        else if (!enemy->isMorphing())
                        {
                            enemy->startDiving(ol::random() < 0.8f); // dive and return -> 80%
                        }
                    }
                    else // guard
                    {
...
void ol::GameController::evaluateChallengingStage()
{
    if (!isFighterDestroyed())
    {
    ...
    setUpdateAction([=] () mutable {
        switch (state)
        {
            ...
            case 1:
                ...
                if (challengingStageTotalEnemiesHit_ == challengingStageTotalEnemiesCount_)
                {
                    playMusic(Music::CHALLENGING_STAGE_PERFECT);
                }
                else
                {
                    playMusic(Music::CHALLENGING_STAGE_MUSIC);
                }

                isNumberOfHitsLabelVisible_ = true;
                waitTime = getGameElapsedTime();
                state = 2;
            ...
            case 2:
                ...
                isNumberOfHitsNumberLabelVisible_ = true;
                waitTime = getGameElapsedTime();
                state = 3;
            ...
            case 3:
                ...
                isBonusLabelVisible_ = true;
                isPerfectLabelVisible_ = challengingStageTotalEnemiesHit_ ==
                                         challengingStageTotalEnemiesCount_;
                ...
                {
                    hudInfo.addScore(100 * challengingStageTotalEnemiesHit_);
                }
            ...
            case 4:
                if (getGameElapsedTime() - waitTime < 6.0f)
                {
                    return;
                }
                isNumberOfHitsLabelVisible_ = false;
    ...
    // log("GameController::evaluateChallengingStage()");
}
